        ...
        <div className="relative flex flex-col justify-center items-center h-screen bg-transparent z-10">
          ...
          {!isComplete && (
            <div className="flex space-x-2 mb-8 fixed top-1/4">
              <div className="loading-dot w-3 h-3 bg-[#272F76] rounded-full"></div>
              <div className="loading-dot w-3 h-3 bg-[#272F76] rounded-full"></div>
              <div className="loading-dot w-3 h-3 bg-[#272F76] rounded-full"></div>
            </div>
          )}
          ...
          <div className="w-11/12 max-w-md mt-48">
            ...
            <div className="space-y-3">
              {loadingSteps.map((step, index) => (
                ...
                >
                  <div className={`status-icon bg-gray-300 text-white rounded-full w-8 h-8 flex items-center justify-center mr-3 ${index <= currentStep ? 'bg-[#272F76]' : ''}`}>
                    {index < currentStep ? (
